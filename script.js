  ...
  Html5Qrcode.getCameras().then(cameras => {
    ...
    html5QrCode.start(
      ...
      (decodedText) => {
        // On successful scan
        if (decodedText) {
          // Try to extract peer ID from URL or plain text
          let peerId = '';
          try {
          ...
          if (peerId) {
            elements.remotePeerId.value = peerId;
            showNotification('Peer ID scanned!', 'success');
            closeQrScanner();
...
document.getElementById('qrcode-container').addEventListener('click', openQrScanner);
closeQrScannerBtn.addEventListener('click', closeQrScanner);
// Also close scanner on modal background click (optional)
