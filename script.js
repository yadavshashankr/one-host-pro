...
async function sendFile(file, peerId) {
    try {
        ...
        if (!conn || !conn.open) {
            throw new Error(`No active connection found for peer: ${peerId}`);
        }

        console.log(`Starting file transfer to peer: ${peerId}`);
        showNotification(`Sending ${file.name} to peer: ${peerId}`, 'info');
